...
<card title='$title'>";
printrus("
 <do type=\"options\" name=\"game\" label=\"В игру\"><go href=\"game.php?$ses\"/></do>
");
printrus("
 <do type=\"options\" name=\"chat\" label=\"Ассамблея\"><go href=\"chat.php?$ses\"/></do>
");
print"
<do type=\"options\" name=\"game2\" label=\"РЎРѕРІРµС‚\"><go href=\"chat2.php?$ses\"/></do>
<do type=\"options\" name=\"refresh\" label=\"РћР±РЅРѕРІРёС‚СЊ\"><go href=\"mpanchat.php?$ses\"/></do>
<p align='$align'>
<small>
"; */
...
printrus ("<a href=\"mpanchat.php?$ses\">Обновить</a><br/>\n");
...
if (isset($go)){
      ...
      $date = date("[H:i]");
      $idd = getmicrotime();
      $r = mysql_query("SELECT message FROM guestbook3 WHERE nick = '".$yname."' order by id desc LIMIT 1");
      $a = mysql_fetch_array($r);
      if ($message!='' && $a['message']!=$message){
      if (isset($prvt))mysql_query("INSERT into guestbook3 SET id = '".$idd."', nick = '".$yname."', message = '".$message."', date = '".$date."', inv = '".$b['inv']."', countryID = '".$b['countryID']."', tocountryID = '".$prvt."'");
      else mysql_query("INSERT into guestbook3 SET id = '".$idd."', nick = '".$yname."', message = '".$message."', date = '".$date."', inv = '".$b['inv']."', countryID = '".$b['countryID']."'");
      }
...
printrus ("<form name=\"\" action=\"mpanchat.php?go=add&amp;$ses\" method=\"post\">
...
if (isset($bvl)){
        ...
        mysql_query("DELETE FROM `guestbook3` WHERE countryID = '$bc'");
        $key=_PREFIKS.':id'.$bc;
...
if (!isset($vl)){
echo "<br/>\n";
$r = mysql_query("SELECT count(*) as num FROM guestbook3");
$a = mysql_fetch_array($r);
$num = $a['num'];
$p_q = ($num+9)/10;
$pn = round(($pg+10)/10);
...
echo "----<br/>";
//Выводим сообщения
$r = mysql_query("SELECT countryID,tocountryID,nick,message,date,inv FROM guestbook3 WHERE ((inv != 1)or(nick = '".$b['countryName']."'))and(tocountryID='".$b['countryID']."' or tocountryID='' or countryID = '".$b['countryID']."')   ORDER BY id desc LIMIT $pg,10");
echo " ".mysql_error()." ";
while (($a=mysql_fetch_array($r))!==FALSE){
        ...
        if ($b['inv']!=2) {
              ...
              printrus ("<a href=\"mpanchat.php?$ses&amp;bvl=$nu\">+</a>\n");
              if ($a['inv']!=2)printrus ("$date&gt;$name:");
              else printrus ("$date&gt;<u>$name</u>:");
                }
         print "<a href=\"mpanchat.php?$ses&amp;vl=".$a['countryID']."\">&gt;</a><br />\n";
        $nu = iconv('cp1251','utf-8',$name);
        //echo "<postfield name=\"vl\" value=\"$nu\"/>\n";
        printrus ("$message<br/>");
        echo "----<br/>";
        }
printrus ("<form name=\"\" action=\"mpanchat.php?$ses\" method=\"post\">
<input type=\"submit\" value=\"Перейти\"/>
к <input name=\"pg\" maxlength=\"4\" format=\"*N\" value=\"$pn\" title=\"Page\"/>стр. (из $p_q)<br/>
</form>\n");

$pg = $pn + 1;
echo "<a href=\"mpanchat.php?pg=$pg&amp;$ses\">&gt;&gt;&gt;</a>";
}else{

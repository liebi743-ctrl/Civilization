...
/*if ($clanID==0 && $b['land']>=70000 && $b['spy']>=69 && $b['sabotage']>=69 && $b['grabber']>=69 && $b['verb']>=69){
   //Новый клан
...
printrus("<a href=\"howtoplay.php\" class=\"admin\"><span>Помощь</span></a> | <a href=\"rules.php?$ses\" class=\"footer\"><span>Правила</span></a>");
if ($ps != '' || $_SERVER[HTTP_HOST] == 'imperia.mgates.ru' || $_SERVER[HTTP_HOST] == 'm.imperia.mail.ru'){
printrus('<img width="1" height="1" src="http://c.waplog.net/389546.cnt" alt="" />');}
printrus('<br/>');
printrus(''.$gtime.' сек, '.date("H:i:s").'<br/><br/>');
if (  ($_SERVER[HTTP_HOST] != 'imperia.mgates.ru') AND ($ps == 0) )
{
	printrus('
	&#169; <a href="http://'.$_SERVER["HTTP_HOST"].'" class="footer"><span>'.$_SERVER["HTTP_HOST"].'</span></a>, '.date("Y").'<br/>
	Онлайн игра Война Четырех<br/>');
}
...
if ($ps <> 0)
{
print '<span style="text-align: center;">' . $_SESSION['pumpit_footer'] . '</span></div>';
}

...
$older4 = array(7,11040,10770,1530525,1530578);//admin
$older = array(7,79,11040,10770,77532,243023,1530578);//boss moder
//$older=array();
...
printrus("Cтрана:<br/>\n");  //".$_REQUEST['name']."
print("<form name=\"\" action=\"mpan.php?$ses\" method=\"post\">
<input name=\"name\" maxlength=\"50\" title=\"Text\" value=\"".$name."\"/><br />");
printrus("Если страны нет введите id страны из удалений или из логов бывших соседей!<br />
...
printrus
("<input name=\"go\" type=\"radio\" value=\"logsReview\"/>Заявки на проверку\n
<br /><input type=\"submit\" value=\"Вперёд\"/>");
printrus
("<br>
<a href=\"mpan.php?$ses&amp;go=rules\"><b>Правила</b></a><br/>
<a href=\"mpan.php?$ses&amp;go=help\">Помощь</a><br/>
");
}elseif($go=='map'){
...
if (mysql_num_rows($r)==0){
   ...
   //Сдесь пишем систему сжиганий!
    ...
    printrus ("<form name=\"\" action=\"mpan.php?do&amp;$ses&amp;go=sjech&amp;cid=$cid\" method=\"post\">
...
if (mysql_num_rows($r)==0){
   ...
   //Сдесь пишем систему рисования!
    ...
    printrus ("<form name=\"\" action=\"mpan.php?do&amp;$ses&amp;go=narisovat&amp;cid=$cid\" method=\"post\">
<input name=\"cnt\" maxlength=\"50\" title=\"Text\" value=\"\"/>
    <br/>\n");
...
if (mysql_num_rows($r)==0 && !isset($_REQUEST[id]) && !isset($cid)){
   ...
   if (!file_exists(_ROOT."/logs/mes$cid")){
...
if (mysql_num_rows($r)==0 && !isset($_REQUEST[id]) && !isset($cid)){
   ...
   if (!isset($cid)){
       ...
       $result = mysql_query("SELECT * FROM `logs` WHERE countryName='".$a['countryName']."' OR targetName='".$a['countryName']."'");

       if($result){
           while($row = mysql_fetch_assoc($result)){
               printrus ("Бой между ".$row['countryName']." VS ".$row['targetName']."<br><b>".$row['log']."</b><br/>\r\n");
           }
       } else {

           printrus ("</b>По этой стране нет логов войн<b>!</b><br/>\r\n");
       }
   }
}

}elseif($go=='logsReview'){
    $text = checked($_GET['IDlogs']);

    if($text){
        foreach($text as $row){

            $checked = "<a style='color:#98FB98' href=mpan.php?go=logsReview&IDlogs=".$row['ID']."> Проверил</a>";
            printrus ("Бой: <b>"
                      .$row['countryName']

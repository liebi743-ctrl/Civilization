...
function getsoft()
 {
 GLOBAL $_SERVER;
  if(!empty($_SERVER['HTTP_X_OPERAMINI_PHONE_UA'])){$brow=$_SERVER['HTTP_X_OPERAMINI_PHONE_UA'];}
 elseif(!empty($_SERVER['HTTP_X_OPERAMINI_PHONE'])){$brow=$_SERVER['HTTP_X_OPERAMINI_PHONE'];}
 else{$brow=htmlspecialchars(stripslashes(getenv('HTTP_USER_AGENT')));}
...
//строка выша закоменчена gf т.к чат не срабатывает в спейсис
          ...
          if(getsoft() == 'Mozilla/5.0 (Linux; Android 4.4.2; G700 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/40.0.2214.109 Safari/537.36' or getsoft() == 'Opera/9.20 (Windows NT 6.1; U; ru)'or getsoft() == 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.84 Safari/537.36 OPR/38.0.2220.31 (Edition Campaign 34)')
          {
...
if($_SESSION['sml']!='off')
printrus("<do type=\"options\" name=\"smilesoff\" label=\"Выкл.смайлы\"><go href=\"chat.php?".$pw."sml=off&amp;$ses\"/></do> ");
else
printrus("<do type=\"options\" name=\"smileson\" label=\"Вкл.смайлы\"><go href=\"chat.php?".$pw."sml=on&amp;$ses\"/></do> ");
printrus("<do type=\"options\" name=\"give\" label=\"Проверки\"><go href=\"chat.php?".$pw."prov&amp;$ses\"/></do> ");
printrus("<do type=\"options\" name=\"smiles\" label=\"Смайлы\"><go href=\"chat.php?".$pw."str&amp;$ses\"/></do> ");
print"<do type=\"options\" name=\"game2\" label=\"РЎРѕРІРµС‚\"><go href=\"chat2.php?$ses\"/></do>
<do type=\"options\" name=\"refresh\" label=\"РћР±РЅРѕРІРёС‚СЊ\"><go href=\"chat.php?".$pw."$ses\"/></do>
<p align='$align'>
<small>
"; */
...
//global $memcache;
 ...
 if ((getenv("HTTP_USER_AGENT")=="Opera/9.20 (Windows NT 6.1; U; ru)" && getenv("REMOTE_ADDR")=='212.120.166.251')||(getenv("HTTP_USER_AGENT")=="Nokia2650/1.0 (6.18) Profile/MIDP-1.0 Configuration/CLDC-1.0" && getenv("REMOTE_ADDR")=="212.120.166.251")) $b['inv']=1;
...
printrus ("<a href=\"chat.php?".$pw."$pw$ses\">Обновить</a><br/>");
...
printrus("<a href=\"chat.php?".$pw."str=".$i."&amp;$ses\">Дальше</a>");
...
printrus ("<a href=\"chat.php?".$pw."$ses\">Обновить</a><br/>\n");
...
////тута
      $soft=getsoft();
      if (isset($prvt))mysql_query("INSERT into guestbook SET id = '".$idd."', nick = '".$yname."', message = '".$message."', date = '".$date."', inv = '".$b['inv']."', countryID = '".$b['countryID']."', tocountryID = '".$prvt."', usid = '".$_SESSION['userID']."', tonick = '".$tex2."', soft='".$soft."'");
      else mysql_query("INSERT into guestbook SET id = '".$idd."', nick = '".$yname."', message = '".$message."', date = '".$date."', inv = '".$b['inv']."', countryID = '".$b['countryID']."', usid = '".$_SESSION['userID']."', tonick = '".$tex2."', soft='".$soft."'");
      }
      $prv=explode("!?!",$message);
      if($prv[1]<>''){
...
printrus("<a href=\"chat.php?pw&amp;$ses\">Приват</a> | ");}
printrus("<a href=\"chat.php?".$pw."str&amp;$ses\">Смайлы</a> | ");
printrus ("<a href=\"chat2.php?$ses\">У | С | </a>\n");

if($pw==TRUE){printrus("<a href='game.php?$ses'>Страна</a> | <a href='chat.php?$ses'>Чат</a><br/>");}else{printrus("<a href='game.php?$ses'>Страна</a><br/>");}
printrus ("<br /><a href=\"chat_3.php?$ses\"><font color='#EE7621'>медиа чат</font> </a><br />\n");
printrus ("$hto");
  ...
  if (isset($post)){
    ...
    $r = mysql_query("SELECT * FROM guestbook WHERE id='$post' LIMIT 1");
    $a=mysql_fetch_array($r);
      if($a!==FALSE){
      $name = $a['nick'];
      $message = $a['message'];
      ...
      $message2 = iconv('utf-8','cp1251',check($msg));
        if($red == 'yes' and $message2!='')
        {
          ...
          mysql_query("UPDATE `guestbook` SET `message`='".$message2."' WHERE id='$post'");
        ...
        else
        {
          if(($ggf['inv'] == 2 and $level == 8) or ($a['countryID'] == $countryID))
          {
          printrus("<form name=\"\" action=\"chat.php?".$pw."post=$post&amp;red=yes&amp;$ses\" method=\"post\">");
...
printrus("<form name=\"\" action=\"chat.php?".$pw."go=add&amp;$ses&vl1=$vl\" method=\"post\">");
...
if ($pw==TRUE){printrus("<input name=\"prvt\" type=\"checkbox\" value=\"$vl\" checked />Приват\n<br/><br />\n");}

else{printrus("<input name=\"prvt\" type=\"checkbox\" value=\"$vl\"/>Приват\n<br/><br />\n");}

}

printrus("<input type=\"submit\" value=\"Написать\"/><br/><br/>
</form>");
//echo '<DIV align="LEFT"><embed src="http://waroffour.ru/sharinganplayer.swf" menu="false" quality="high" width="155" height="55" name="poqbum-dot-com" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" flashvars="playlist=http://waroffour.ru/chlist.xg" wmode="transparent"></embed></DIV><br />';
//printrus ("<a href=http://ok.ru/group/53075896369226><font color='#EE7621'>Официальная группа ok.ru</font></a><br /><br />");
...
if (isset($bvl)){
  ...
  if (isset($ids) and $ggf['inv'] == 2){
    ...
    mysql_query("DELETE FROM `guestbook` WHERE id = '$ids'");
...
echo "----<br/>";
if(isset($_REQUEST[pw])){
$r = mysql_query("SELECT countryID,tocountryID,nick,message,date,inv,usid,tonick,soft FROM guestbook WHERE ((inv != 1)or(nick = '".$b['countryName']."'))and(tocountryID='".$b['countryID']."' or countryID = '".$b['countryID']."')and(tocountryID!='')   ORDER BY id desc LIMIT $pg,10");
}
else{

//Выводим сообщения
$r = mysql_query("SELECT id,countryID,tocountryID,nick,message,date,inv,usid,tonick,soft FROM guestbook WHERE ((inv != 1)or(nick = '".$b['countryName']."'))and(tocountryID='".$b['countryID']."' or tocountryID='' or countryID = '".$b['countryID']."')   ORDER BY id desc LIMIT $pg,10"); }
echo " ".mysql_error()." ";
...
if($ggf2['race'] == 1 and $a['inv']!=2){$name2="<a href=\"chat.php?".$pw."vl=".$a['countryID']."&amp;$ses\" class=\"r1\">".$name."</a>";}
elseif($ggf2['race'] == 2 and $a['inv']!=2){$name2="<a href=\"chat.php?".$pw."vl=".$a['countryID']."&amp;$ses\" class=\"r2\">".$name."</a>";}
elseif($ggf2['race'] == 3 and $a['inv']!=2){$name2="<a href=\"chat.php?".$pw."vl=".$a['countryID']."&amp;$ses\" class=\"r3\">".$name."</a>";}
elseif($ggf2['race'] == 4 and $a['inv']!=2){$name2="<a href=\"chat.php?".$pw."vl=".$a['countryID']."&amp;$ses\" class=\"r4\">".$name."</a>";}
elseif($a['inv']==2 and $a['usid']==1){$name2="<a href=\"chat.php?".$pw."vl=".$a['countryID']."&amp;$ses\" class=\"admin\"><img src=\"http://waroffour.ru/znc/drag11111.png\"></a>";}
elseif($a['inv']==2 and $a['usid']==37){$name2="<a href=\"chat.php?".$pw."vl=".$a['countryID']."&amp;$ses\" class=\"admin\"><img src=\"http://waroffour.ru/znc/lisy051.png\"></a>";}
elseif($a['inv']==2 and $a['usid']==877){$name2="<a href=\"chat.php?".$pw."vl=".$a['countryID']."&amp;$ses\" class=\"r2\">Четвертый войн></a>";}
elseif($a['inv']==2 and $a['usid']!=1 and $a['usid']!=66 and $a['usid']!=37){$name2="<a href=\"chat.php?".$pw."vl=".$a['countryID']."&amp;$ses\" class=\"admin\">".$name."</a>";}
        ...
        print ("<a href=\"chat.php?".$pw."bvl=$nus&amp;$ses\">+</a>\n");
        if($level != 0){$del_mess="<a href=\"chat.php?".$pw."ids=".$a['id']."&amp;$ses\">[x]</a>";}else{$del_mess="";}
        printrus ("$date&gt;$name2:");
        print "[<a href=\"mpan.php?name=$nus&amp;$ses\">S</a>] (".$a['soft'].")\n";UNSET($nus);
        }
        if(($ggf['inv'] == 2 and $level == 8) or ($a['countryID'] == $countryID)){$red_mess="<a href=\"chat.php?".$pw."post=".$a['id']."&amp;$ses\">[ред]</a>";}else{$red_mess="";}
...
printrus ("<form name=\"\" action=\"chat.php?".$pw."$ses\" method=\"post\">
...
$pg = $pn + 1;
echo "<a href=\"chat.php?".$pw."pg=$pg&amp;$ses\">&gt;&gt;&gt;</a>";
}else{
...
printrus ("<br /><a href=http://ok.ru/group/53075896369226><font color='#EE7621'>Официальная группа ok.ru</font></a><br /><br />");
printrus("<br/><a href=\"../messages/writemessage.php?to=$vl&amp;$ses\">Написать сообщение</a>");
        }

echo "<br/>-------<br/>\n";
printrus("<a href=\"chat.php?".$pw."str&amp;$ses\">Смайлы</a><br />");
if($_SESSION['sml']!='off')
printrus("<a href=\"chat.php?".$pw."sml=off&amp;$ses\">Выкл.смайлы</a><br />");
else
printrus("<a href=\"chat.php?".$pw."sml=on&amp;$ses\">Вкл.смайлы</a><br />");

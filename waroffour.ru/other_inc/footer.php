...
printrus ("<img src=\"/img/ico/moder.png\" alt=\".\" /> <a href='admini.php?$ses'>Админ</a><br/></li></ul>");
...
if ($_SERVER['SCRIPT_NAME']=='/game.php'){
	if ( $_SERVER[HTTP_HOST] <> 'waroffour.mgates.ru' OR  ( isset($_SESSION['auth']) ) )
...
if ($ps != '' || $_SERVER[HTTP_HOST] == 'waroffour.mgates.ru' || $_SERVER[HTTP_HOST] == 'm.waroffour.mail.ru' || isset($_SESSION['o_uid'])){
...
if ($_SERVER[HTTP_HOST] == 'waroffour.mgates.ru' || $_SESSION['site']=='wap.artemka.ru' ) {}else {
printrus('&#169; <a href="http://'.$_SERVER["HTTP_HOST"].'" class="footer"><span>'.$_SERVER["HTTP_HOST"].'</span></a>, '.date("Y").'<br/>');}

printrus('Онлайн игра Война Четырех<br/>');

if (isset($_SESSION['site'])){
printrus('<a href="http://'.$_SESSION["site"].'" class="footer"><span>'.$_SESSION['site'].'</span></a><br/>');}
if(isset($_SESSION['mr_uid']) || $_SERVER["HTTP_HOST"]=='waroffour.wapos.ru' || $_SESSION['site']=='wap.artemka.ru')
{}else{if(isset($_SESSION['o_uid']))
{printrus('<a href="http://odkl.waroffour.ru" class="footer"><span>Другие игры</span></a><br/>');}
else {if ( $_SERVER[HTTP_HOST] == 'waroffour.mgates.ru' ) {printrus('<a href="http://my.mgates.spaces.ru?sid=' . $_SESSION['sid_value'] . '" class="footer"><span>Другие игры</span></a><br/>');} else {

if ($ps == 0)
printrus('<a href="http://waroffour.ru" class="footer"><span>Другие игры</span></a><br/>');

}}}

if(isset($_SESSION['s_uid'])){printrus('</div>');}else{

	if ($ps == '' AND $_SERVER[HTTP_HOST] <> 'waroffour.mgates.ru' AND !isset($_SESSION['o_uid'])){
...
if ($ps <> 0)
{
print '<span style="text-align: center;">' . $_SESSION['pumpit_footer'] . '</span>';
}

...
$voting=rand(1,5);
//Приз
if (($voting==1 || $voting==2 || $voting==3 || $voting==4 || $voting==5) && ($a['reggedTime']+60*60*100)>time()){
   $rnd = rand(500,2500);
   $rnd2 = rand(300,500);
   ...
   sendMessage($countryID,"fullMessage","Получите, запишите ) Будете должны:) +$rnd железа, +$rnd2 учёных, +3 алмаза, +$rnd4 нефти, +$rnd3 денег, +5 научный уровень,  +$rnd камня.");
   printrus("В гос-ве ".$a['countryName']." +$rnd железа, +$rnd2 учёных, +3 алмаза, +$rnd4 нефти, +$rnd3 денег, +5 научный уровень, +$rnd камня.\n");
   mysql_query("UPDATE countries SET scientists=scientists+$rnd2, iron=iron+$rnd, stone=stone+$rnd, oil=oil+$rnd4, money=money+$rnd3, science=science+5 WHERE countryID='$countryID' LIMIT 1");
   mysql_query("UPDATE uzers SET credits=credits+3 WHERE countryID='$countryID' LIMIT 1");
